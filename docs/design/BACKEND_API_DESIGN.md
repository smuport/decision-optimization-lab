...
Day6 已实现全部教师接口。只有 DRAFT 可编辑；状态严格按 `DRAFT → PUBLISHED → CLOSED → ARCHIVED` 单向流转。发布先检查 Exercise、CaseRelease、时间、次数和六项资源完整性，再在数据库事务中重新校验 Assignment/Exercise/CaseRelease 状态并写入 `publishedAt`。
...
Day6 已实现列表与详情。详情同时返回公开数据集、默认模板内容、公开 output schema、rubric 摘要、`canSubmit`、`isLate`、剩余次数和 Assignment 资源下载地址；DRAFT 不向学生暴露，CLOSED/ARCHIVED 仍可读取历史内容。

#### Exercise 资源与提交

```text
GET  /api/v1/exercises/:exerciseId/resources/download
GET  /api/v1/assignments/:assignmentId/resources/download
...
Day6 学生工作区使用 Assignment 资源路由。提交记录的次数分配、迟交标记和最大次数校验在同一数据库事务内完成；UPCOMING、截止后不允许迟交、CLOSED、ARCHIVED 和次数耗尽均拒绝创建 Submission。Exercise 资源路由仅作为旧页面兼容入口保留。
